import os
import sys
import json

# Initialise variables here with default values
waitTime = "1"
exitStatus = "False"

def program(settings):
    # Put script code here
    import time
    time.sleep(int(waitTime))
    return bool(exitStatus)  # End Script (True = Finished, False = Error)


def parse_args(args):
    settings = {}
    for arg in args:
        if "=" in arg:
            key, value = arg.split("=", 1)
            settings[key] = value
    return settings


def main():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    if os.path.basename(os.getcwd()) == 'switch-control':
        json_dir = os.path.abspath(os.path.join(base_dir, "..", "data"))
    else:
        json_dir = os.path.abspath(os.path.join(base_dir, "../../../data"))

    # Retrieve settings
    settings = parse_args(sys.argv[1:])
    for key, value in settings.items():
        globals()[key] = value

    print("Program Settings:")
    for key in settings:
        print(f"{key} =  {globals().get(key)}")

    # Run program and determine handle failure/success
    status = program(settings)
    status_message = "Finished" if status else "Error"

    status_file_path = os.path.join(json_dir, "status.json")
    if os.path.exists(status_file_path):
        with open(status_file_path) as status_file:
            status_data = json.load(status_file)
    else:
        status_data = {}

    status_data['status'] = status_message

    # Write to JSON
    try:
        print(f"writing status data to {status_file_path}")
        with open(status_file_path, "w") as status_file:
            json.dump(status_data, status_file, indent=2)
    except Exception as e:
        print(e)


if __name__ == "__main__":
    main()
